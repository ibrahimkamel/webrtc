...
var WebrtcConnection = function(userName, channelName, pcConfig, startCallBtn, endCallBtn, muteCallBtn, localVideoDiv,
    remoteVideoDiv, callType, chatSendBtn, chatMessageInput, chatMessageDiv, localVideoScreenDiv,
    remoteVideoScreenDiv, shareScreenBtn, fileShareDiv)
{
    ...
    var reset = function()
    {
        ...
        if (localVideoScreenStream)
        {
            localVideoScreenStream.getVideoTracks()[0].stop();
            localVideoScreenStream.getAudioTracks()[0].stop();
        }
    ...
    var InitiateConnections = function()
    {
        if (ably && ably.auth && ably.auth.tokenDetails)
        {
            ...
            sendMessage(
            {
                "msg": userName + " has joined the meeting.",
                'type': "msg"
            });
    ...
    var displayStats = function()
    {
        ...
        if (DataPeerConnection && DataPeerConnection.iceConnectionState === 'connected')
        {
                    ...
                    for (var key in stats)
                    {
                        ...
                        if (res.type === 'googCandidatePair' && res.googActiveConnection === 'true')
                        {
                            ...
                            var bitrate = Math.round(
                                (bytesNow - bytesPrev) * 8 / (res.timestamp - timestampPrev));
                            display(bitrate);
            ...
            else
            {
                ...
                var bitrate = Math.round(
                    (bytesNow - bytesPrev) * 8 / (now - timestampPrev));
                display(bitrate);
                timestampPrev = now;
                bytesPrev = bytesNow;
                if (bitrate > bitrateMax)
